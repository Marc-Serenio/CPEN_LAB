...
class mainUI:
    ...
    def mainscreen1(self):
        ...
        if BUTTONOPT1:
            self.LOGOBUTTON7 = Button(root, image=BUTTONOPT1, bg="#f77f00", relief="sunken")
        ...
        self.back_button = Button(root, text="Back", command=self.mainscreen, font=('Poppins', 18, 'bold'), fg='white', width=20, bg='#4b3621', height=2, borderwidth=4, relief='sunken')
    ...
    def summarytable(self):
        # Create a separate thread to run the long-running task
    ...
    def staticmainInput1_window(self):
        ...
        self.STATIC_header = Button(root, text="Header Image Placeholder", bg="#000000", relief="flat", borderwidth=0)
    ...
    def STATIC_show_partition_input(self):
        ...
        self.STATIC_submit_button = Button(root, text="Submit", command=self.STATIC_create_job_entries, font=('Poppins', 15, 'bold'), width=20, bg="#b8804c", height=2, borderwidth=4, relief="sunken")
    ...
    def STATIC_create_job_entries(self):
        ...
        for i in range(self.STATIC_num_jobs):
            ...
            STATIC_job_frame.place(x=50, y=110 + (i * 50))
            ...
            STATIC_job_size_label.grid(row=0, column=0, padx=5, pady=5)
            STATIC_job_size_entry = Entry(STATIC_job_frame, font=('Poppins', 14, 'bold'), justify="center", width=10)
            STATIC_job_size_entry.grid(row=0, column=1, padx=5, pady=5)
            ...
            STATIC_job_arrival_label.grid(row=0, column=2, padx=5, pady=5)
            STATIC_job_arrival_entry = Entry(STATIC_job_frame, font=('Poppins', 14, 'bold'), justify="center", width=10)
            STATIC_job_arrival_entry.grid(row=0, column=3, padx=5, pady=5)
            ...
            STATIC_job_run_label.grid(row=0, column=4, padx=5, pady=5)
            STATIC_job_run_entry = Entry(STATIC_job_frame, font=('Poppins', 14, 'bold'), justify="center", width=10)
            STATIC_job_run_entry.grid(row=0, column=5, padx=5, pady=5)

            self.STATIC_job_entries.append((STATIC_job_size_entry, STATIC_job_arrival_entry, STATIC_job_run_entry))

        self.STATIC_start_button = Button(root, text="Start Computing", command=self.STATIC_get_main_input1, font=('Poppins', 18, 'bold'), width=20, bg="#b8804c", height=2, borderwidth=4, relief="sunken")
        self.STATIC_start_button.place(x=330, y=30)
    ...
    def display_static_table(self, mainProcess1):
        ...
        self.partition_label.grid(row=0, columnspan=4, pady=10)

        self.headers = ["Partition Number", "Partition Size", "Status", "Job"]
        for col_num, header in enumerate(self.headers):
            header_label = Label(self.partition_table, text=header, font=('Poppins', 10, 'bold'), bg="#eec894",
        ...
        # Create a frame for navigation controls
        self.nav_frame = Frame(root)
        self.nav_frame.pack(pady=10)

        # Add Previous button, timestamp label, and Next button to the frame
    ...
    def display_partition_info(self, mainProcess1, timestamp):
        ...
        partition_info = mainProcess1.partition_history[timestamp]
        for row_num, (partition, info) in enumerate(partition_info.items(), start=2):
            ...
            partition_job_label = Label(self.partition_table, text=info["job"], font=('Poppins', 10, 'bold'), bg="#FFFFFF", width=15, borderwidth=4, relief="raised")
    ...
    def navigate_partitions(self, mainProcess1, direction):
        ...
        if 0 <= new_timestamp <= self.max_timestamp:
            ...
            self.timestamp_label.config(text=f"Timestamp: {self.current_timestamp}")
...
class processingUI1:
    ...
    def STATIC_display_partition_status(self):
        ...
        self.partition_history.append(STATIC_partition_info)
        print(STATIC_partition_info)
    ...
    def STATIC_allocate_job_to_partition(self, STATIC_job, STATIC_partition_index):
        ...
        self.STATIC_job_allocations[STATIC_partition_index] = STATIC_job
    ...
    def STATIC_deallocate_job_from_partition(self, STATIC_partition_index):
        ...
        if STATIC_partition_index in self.STATIC_job_allocations:
            ...
            while not allocated:
                for i, STATIC_partition_size in enumerate(self.STATIC_partition_sizes):
                    if self.STATIC_partition_status[i] == 'Free' and STATIC_job_size <= STATIC_partition_size:
                        ...
                        self.STATIC_display_partition_status()
