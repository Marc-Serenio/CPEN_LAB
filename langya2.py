...
class mainUI:
    ...
    def run_main_processing(self, STATIC_os_size, STATIC_memory_size, STATIC_partition_sizes, STATIC_jobs):
        ...
        self.create_memory_map(memory_map, os_size,  memory_size)
    ...
    def slider_moved(self, event, mainProcess1):
        ...
        self.create_memory_map(memory_map)
        ...
        if 0 <= new_timestamp <= self.max_timestamp:
            ...
            for partition_number, partition_info in partition_history['partitions'].items():
                if 'memory_map' in partition_info:
                    self.create_memory_map(partition_info['memory_map'])
                    break
            else:
    ...
    def display_table(self, df):
        ...
        for row_num, job in enumerate(self.jobs, start=1):
    ...
    def create_memory_map(self, mainProcess1, memory_map, os_size, memory_size):
        ...
        self.time_label = Label(self.memory_map_frame, text="Time Slider", font=('Poppins', 14, 'bold'), bg="#ecd3bf")
        self.time_label.place(x=560, y=600)
        self.basicWidgetList.append(self.time_label)


        self.update_button = Button(self.memory_map_frame, text="Update Memory Map", command=self.update_memory_map,
                                    font=('Poppins', 14, 'bold'), bg="#b8804c", width=20, height=2, borderwidth=4,
                                    relief="sunken")
        self.update_button.place(x=560, y=650)
    ...
    def update_memory_map(self, *args):
        try:
            ...
            self.canvas.create_rectangle(50, 0, 250, 500, outline="black")

            os_height = (self.os_size / self.memory_size) * 500
            self.canvas.create_rectangle(50, 0, 250, os_height, fill="gray", outline="black")
            self.canvas.create_text(150, os_height / 2, text="OS", fill="black")

            current_time = int(self.time_slider.get())

            partitions = self.static_partition(self.memory_size - self.os_size)

            y = os_height
            for i, partition in enumerate(partitions):
                partition_height = (partition / self.memory_size) * 500
                self.canvas.create_rectangle(50, y, 250, y + partition_height, fill="blue" if i % 2 == 0 else "green",
                                             outline="black")
                self.canvas.create_text(150, y + partition_height / 2, text=f"Partition {i + 1}", fill="black")
                y += partition_height
...
class processingUI1:
    def __init__(self, STATIC_os_size, STATIC_memory_size, STATIC_partition_sizes, STATIC_jobs):
        ...
        self.partition_history = []  # Initialize partition history as an empty list
        self.STATIC_job_allocations = {}
        self.partition_history = []


    def static_partition(self, memory_size):
        partitions = []
        while memory_size > 0:
            partition_size = min(memory_size, 100)  # fixed partition size of 100
            partitions.append(partition_size)
            memory_size -= partition_size
        return partitions

    def slider_moved(self, event, mainProcess1):
        try:
            memory_map = mainProcess1.partition_history[self.current_timestamp].get('memory_map', {})
            self.create_memory_map(memory_map)
        except KeyError:
            # Handle the case where 'memory_map' key is missing
            print("Error: 'memory_map' key not found in partition history at current timestamp.")
    ...
    def update_partition_history(self, mainProcess1, memory_map):
        for timestamp in mainProcess1.partition_history:
            timestamp['memory_map'] = memory_map
